...
void main() {
  ...
  test('produto simples usa disponibilidade real no PDV', () async {
    ...
    expect(entries.single.totalPhysicalStockMil, 10000);
    expect(entries.single.totalReservedStockMil, 3000);
    expect(entries.single.totalStockMil, 7000);
    expect(entries.single.buildSellableProduct().stockMil, 7000);
  });

  test('variante usa disponibilidade real no seletor do PDV', () async {
    final fixture = await _openFixture();
    addTearDown(fixture.dispose);
    ...
    expect(entries, hasLength(1));
    final option = entries.single.availableVariants.single;
    expect(option.variant.id, 10);
    expect(option.physicalQuantityMil, 5000);
    expect(option.reservedQuantityMil, 2000);
    expect(option.availableQuantityMil, 3000);
    expect(entries.single.buildSellableVariantProduct(option).stockMil, 3000);
  });

  test('variante totalmente reservada nao aparece no PDV', () async {
    final fixture = await _openFixture();
    addTearDown(fixture.dispose);
    await fixture.insertVariantProduct(
      parentStockMil: 1000,
  ...
  test(
    'reservas released e converted nao reduzem disponibilidade no PDV',
    () async {
      final fixture = await _openFixture();
      addTearDown(fixture.dispose);
      await fixture.insertSimpleProduct(stockMil: 10000);
      ...
      expect(entries, hasLength(1));
      expect(entries.single.totalReservedStockMil, 0);
      expect(entries.single.totalStockMil, 10000);
    },
  );

  test(
    'reserva da variante A nao reduz disponibilidade da variante B no PDV',
    () async {
      final fixture = await _openFixture();
      addTearDown(fixture.dispose);
      await fixture.insertVariantProduct(
      ...
      final entries = await container.read(salesCatalogProvider.future);
      final byVariantId = {
        for (final option in entries.single.availableVariants)
  ...
  test('produto simples sem disponibilidade real nao aparece no PDV', () async {
    final fixture = await _openFixture();
    addTearDown(fixture.dispose);
    await fixture.insertSimpleProduct(stockMil: 1000);
    await fixture.insertReservation(productVariantId: null, quantityMil: 1000);
    final container = fixture.createContainer();
    addTearDown(container.dispose);
...
Future<_SalesAvailabilityFixture> _openFixture() async {
  final isolationKey =
      'remote:sales-catalog-availability-${DateTime.now().microsecondsSinceEpoch}';
  final appDatabase = AppDatabase.forIsolationKey(isolationKey);
  final database = await appDatabase.database;
  return _SalesAvailabilityFixture(
...
class _SalesAvailabilityFixture {
  const _SalesAvailabilityFixture({
