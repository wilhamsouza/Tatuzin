...
describe("tenant deletion persisted workflow", () => {
  ...
  it("marca identity pending e verifica identidade com atores persistidos", async () => {
    ...
    assert.equal(
      fixture.requests[0]?.identityVerifiedByAdminUserId,
      "admin-2",
    );
    assert.deepEqual(
  ...
  const client: Record<string, unknown> = {
    ...
    tenantDeletionRequest: {
      async findMany(input: Record<string, unknown>) {
        ...
        return requests
          .filter((request) => {
            if (where.companyId != null && request.companyId !== where.companyId) {
              return false;
            }
      ...
      async findUnique(input: { where: { id: string } }) {
        return requests.find((request) => request.id === input.where.id) ?? null;
      },
...
function persistedRequestFixture(
  ...
  return {
    ...
    status: (data.status ?? "REQUESTED") as TenantDeletionPersistedRequest["status"],
    ...
    createdAt,
    updatedAt:
      data.updatedAt instanceof Date ? data.updatedAt : createdAt,
    company,
    auditEvents: [],
  };
}
